...
- [2026-06-10] (promoted) **When writing ANY code that calls an LLM** (gates, writers, extractors, scorers) → never import a vendor SDK directly; route through `scripts/llm_client.py` so the task runs with whatever provider key the operator has (Gemini/Anthropic/OpenAI, `KAI_LLM_PROVIDER`/`KAI_LLM_MODEL` to pin). Only exemption: Gemini-native multimodal transcription (EC-19). Source: human correction on four_us_score.py, generalized to all call sites. Enforced: scripts/llm_client.py + tests/test_llm_client.py (vendor-import lint on pipeline files)
