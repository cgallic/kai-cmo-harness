...
- **O** — metric, source, baseline, threshold, window, and owner recorded *before* ship. A baseline written after ship is rejected.

This skill produces the artifact and can reach C. It does not publish, and it never issues its own verdict.
...
- **One primary CTA.** Word count comes from the contract, not from feel.
- **SEO content applies Algorithmic Authorship:** conditions after the main clause ("Do X if Y"), instructions start with verbs, sentences under 20 words, bold the answer rather than query-matching terms.
...
| TikTok script | `knowledge/channels/tiktok-algorithm.md` · `knowledge/checklists/tiktok-checklist.md` |
...
- Ad copy is requested for a platform with no policy reference in `harness/references/`.
- A claim the copy depends on cannot be sourced — a quantitative claim with no source does not ship.
- Cold outreach is requested where the consent basis or suppression list is unknown.
