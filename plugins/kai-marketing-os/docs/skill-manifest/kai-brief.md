...
- `topic` (string, required) - content topic, campaign angle, or asset request.
- `format` (string, required) - intended downstream artifact type.
...
### Outputs
- Artifact -> structured brief that downstream skills such as `kai-write` and `kai-email-system` can consume.
...
### Example artifacts
- [Northstar Media Lab brief](../../workspace/demo-clients/northstar-media-lab/brief.md)
